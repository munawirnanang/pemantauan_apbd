  ...
  <body>
    ...
    <script>
      ...
      var dynamic_indonesia_map = L.esri
        .dynamicMapLayer({
          url: "https://geospasial.bappenas.go.id/server/rest/services/Produksi/PEPPD_ADHB_per_Kapita_Provinsi/MapServer",
          opacity: 0.7,
        })
        .addTo(map);
      ...
      var feature_indonesia_map = L.esri
        .featureLayer({
          url: "https://geospasial.bappenas.go.id/server/rest/services/Produksi/PEPPD_ADHB_per_Kapita_Provinsi/FeatureServer/0",
          simplifyFactor: 0.35,
          precision: 5,
      ...
      /*var latLngArea;
      feature_indonesia_map.on('click', function(ev) {
	    console.log(ev.layer._latlngs.length); // ev is an event object (MouseEvent in this case)
	    console.log(ev.layer._latlngs); // ev is an event object (MouseEvent in this case)

	    feature_indonesia_map.resetStyle(latLngArea);
	    
	    latLngArea = ev.layer._latlngs;

	    // create a red polygon from an array of LatLng points
		var polygon = L.polygon(latLngArea, {color: 'red'}).addTo(map);
	  });*/

      // Inisialisasi variable areaId untuk menampung id provinsi
      var areaId;

      // Apabila mouse mengklik layer maka
      feature_indonesia_map.on("click", function (e) {
        // console.log(e);

        // cek apakah value e.layer.feature.id sama dengan areaId, jika sama
        if (e.layer.feature.id == areaId) {
        	// Menghapus style warna layer
	        feature_indonesia_map.resetFeatureStyle(areaId);
	        // kosongkan/reset value areaId;
	        areaId = '';
	        //togglePopup, jika popup open maka lakukan close, begitupun sebaliknya
	        feature_indonesia_map.bindPopup().togglePopup();
	    }else{
	    	// Menghapus style warna layer
	    	feature_indonesia_map.resetFeatureStyle(areaId);
	    	// Masukan id provinsi ke var areaId
	        areaId = e.layer.feature.id;
      ...
      feature_indonesia_map.on("mouseout", function (e) {
        ...
        // cek apakah value oldId tidak sama dengan areaID, jika sama
        if (oldId != areaId) {
	        // Menghapus style warna layer
    ...
    <script>
      ...
      $.getJSON(
        "https://geospasial.bappenas.go.id/server/rest/services/Produksi/PEPPD_ADHB_per_Kapita_Provinsi/MapServer/legend?f=pjson",
        function (data) {
    ...
    <script>
      ...
      $.getJSON(
        "https://geospasial.bappenas.go.id/server/rest/services/Produksi/PEPPD_ADHB_per_Kapita_Provinsi/FeatureServer?f=pjson",
        function (data) {
