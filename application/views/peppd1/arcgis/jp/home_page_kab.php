  ...
  <body>
    <div class="row m-2">
      <div class="col-sm-4 p-1">
        <div class="row">
          <div class="col-6 pr-1">
            ...
            <div
              class="card border-dark mb-2"
              style="background-color: #33b86c; height: 155px;"
            >
              <div class="card-body text-light p-2">
                <h6 style="font-size: 0.9rem;">Jumlah Kabupaten / Kota</h6>
                <h1 style="font-size: 2rem;" id="jumlah_kab_kota"></h1>
                <h6 style="font-size: 0.9rem;">Kabupaten / Kota</h6>
              </div>
            </div>
          </div>
          <div class="col-6 pl-1">
            <div
              class="card border-dark mb-2"
              style="background-color: #33b86c; height: 220px;"
            >
              <div class="card-body text-light p-2" style="overflow-y: auto;">
                <!-- Tampilan legenda -->
                <div id="legend" style="font-size: 0.9rem;"></div>
                <div id="satuan" style="font-size: 0.9rem;"></div>
              </div>
            </div>
          </div>
        </div>
        <div class="row">
          <div class="col-12">
            <div
              class="card border-dark mb-2"
              style="background-color: #33b86c; height: 330px;"
            >
      ...
      <div class="col-sm-8 p-1">
        <div class="card border-dark" style="height: 560px">
          <div class="card-body">
            <!-- Tampilan Map -->
            <div id="map"></div>
            <!-- Tampilan info kab/kota - Provinsi -->
    ...
    <script>
      ...
      var dynamic_indonesia_map = L.esri
        .dynamicMapLayer({
          url: "https://geospasial.bappenas.go.id/server/rest/services/Produksi/PEPPD_Jumlah_Penganggur_KabupatenKota/MapServer",
          opacity: 0.7,
        })
        .addTo(map);
      ...
      var feature_indonesia_map = L.esri
        .featureLayer({
          url: "https://geospasial.bappenas.go.id/server/rest/services/Produksi/PEPPD_Jumlah_Penganggur_KabupatenKota/FeatureServer/0",
          simplifyFactor: 0.35,
          precision: 5,
      ...
      feature_indonesia_map.on("mouseover", function (e) {
        ...
        document.getElementById("info-pane").innerHTML =
          e.layer.feature.properties.wadmkk +
          " - " +
      ...
      feature_indonesia_map.bindPopup(function (layer) {
        console.log(layer);
        return (
          "<strong>Jumlah Penganggur</strong><hr />" +
          "<span>Kab/Kota : <strong>" +
          layer.feature.properties.wadmkk +
          "</strong></span><br />" +
          "<span>Provinsi : <strong>" +
          layer.feature.properties.wadmpr +
          "</strong></span><br />" +
          "<span>Capaian: <strong>" +
          layer.feature.properties.jmpeng_2020 +
          "</strong></span><br />" +
      ...
      feature_indonesia_map.on("load", function (e) {
        var count_kabkota = Object.keys(e.target._layers).length;
        // console.log(count_kabkota);
        document.getElementById("jumlah_kab_kota").innerHTML = count_kabkota;
      });
    ...
    <script>
      ...
      $.getJSON(
        "https://geospasial.bappenas.go.id/server/rest/services/Produksi/PEPPD_Jumlah_Penganggur_KabupatenKota/MapServer/legend?f=pjson",
        function (data) {
          // console.log(data.layers[0]);
          // console.log(data.layers[0].legend.length);
          // console.log(data.layers[0].legend[0]['label']);
          let title_legend = data.layers[0].layerName;
          let title_legend_replace = title_legend.replace(/[()]/g,'');
          var html = "<strong>" + data.layers[0].layerName + "</strong><hr/>";
          // var html = "<strong>" + title_legend_replace + "</strong><div id='satuan'></div><hr/>";
          // var html = "<strong>" + title_legend_replace + "</strong><hr/>";
          ...
          for (let index = 0; index < data.layers[0].legend.length; index++) {
            ...
            html +=
              ...
              data.layers[0].legend[index]["label"].toString().replace(/\B(?=(\d{3})+(?!\d))/g, ".") +
              "</span>";
          }
          html += "</ul>";
          $("#legend").append(html);
    ...
    <script>
      ...
      $.getJSON(
        "https://geospasial.bappenas.go.id/server/rest/services/Produksi/PEPPD_Jumlah_Penganggur_KabupatenKota/FeatureServer?f=pjson",
        function (data) {
          ...
          let indexSatuanStart1 = serviceDesc.indexOf("(Satuan:");
          //cari nomor index kata ) dari kata indexSatuanStart1
          let indexSatuanEnd1 = serviceDesc.indexOf(")", indexSatuanStart1);
          
          //potong(slide) dari kalimat indexSatuanStart1 pada kalimat serviceDesc
          let descSatuanSlice = serviceDesc.slice(indexSatuanStart1);  
          ////ubah(replace) dari kalimat descSatuanSlice menjadi "" pada kalimat serviceDesc
          let descDefinisi = serviceDesc.replace(descSatuanSlice, "");
          //potong(slice) dari kalimat indexSatuanStart1 sampai indexSatuanEnd1+1 pada kalimat serviceDesc
          let descSatuan1 = serviceDesc.slice(indexSatuanStart1, indexSatuanEnd1+1);
          //potong(slice) dari kalimat indexSatuanStart1+1 sampai indexSatuanEnd1 pada kalimat serviceDesc
          let descSatuan2 = serviceDesc.slice(indexSatuanStart1+1, indexSatuanEnd1);
          //cari nomor index kata Capaian Nasional:
          let indexCapaianStart1 = descSatuanSlice.indexOf("Capaian Nasional:");
          //potong(slice) dari kalimat indexCapaianStart1 pada kalimat descSatuanSlice
          let descCapaian1 = descSatuanSlice.slice(indexCapaianStart1);
          //ubah(replace) dari kalimat descSatuan1 menjadi descSatuan2 pada kalimat descSatuanSlice
          let descReplace1 = descSatuanSlice.replace(descSatuan1, descSatuan2);
          //ubah(replace) dari kalimat descCapaian1 menjadi "" pada kalimat descSatuanSlice
          let descReplace2 = descReplace1.replace(descCapaian1, "");
          //gabung kata descCapaian1 dengan kalimat descReplace2
          let descReplace2Capaian = descDefinisi+descCapaian1+"\n\n"+descReplace2;
          //gabung kalimat descSlice2 dengan descReplace2Capaian
          let descAll = descReplace2Capaian;
