    ...
    public function index(){
        if($this->input->is_ajax_request()){
            ...
            {                
                ...
                //common properties
                $this->js_init    = "main";
                $this->js_path    = "assets/js/admin/management/users_pemantauan.js?v=".now("Asia/Jakarta");
        ...
        if($this->input->is_ajax_request()){
            try {
                ...
                $tambah="INSERT INTO tbl_user (userid,        `name`,                                `password`,               `unit_code`,`groupid`,                         `active_flag`,`cr_dt`,             `cr_by`)
                   VALUES                     ('".$userid."', '".$this->input->post("name")."', '".md5("pancasila"."monda")."','',        '".$this->input->post("group")."','Y',          '$current_date_time','".$this->session->userdata(SESSION_LOGIN)->userid."')";
    ...
    public function detail_act()
    {
        if($this->input->is_ajax_request()){
            try {
                ...
                $this->form_validation->set_rules('groupuser','Group','required|xss_clean');
                ...
                $list_data = $this->m_ref->get_by_condition($select,$cond);
                    if($list_data->num_rows() == 0){
                        $tambah="INSERT INTO tbl_user (userid,        `name`,                                `password`,               `unit_code`,`groupid`,                         `active_flag`,`cr_dt`,             `cr_by`)
                        VALUES                     ('".$iduser."', '".$this->input->post("nama")."', '".$pass."','',        '".$this->input->post("groupuser")."','".$this->input->post("stts")."',          '$current_date_time','".$this->session->userdata(SESSION_LOGIN)->userid."')";
              ...
              SET
                `group` = '".$this->input->post("groupuser")."',
                `active_flag` = '".$this->input->post("stts")."'
              WHERE `id` = '".$userid."'";
