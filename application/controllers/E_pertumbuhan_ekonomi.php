...
class E_pertumbuhan_ekonomi extends CI_Controller
{
            ...
            try {
                ...
                if ($pro == '' && $rpjmn=='') {
                    ...
                    foreach ($list_thn->result() as $Lis_thn) {
                        $periode                = $Lis_thn->periode;
                        if ($periode == '00') {
                            $thn = $Lis_thn->thn; //$thn2=$row->tahun; 
                        } else {
                            $thn =  $prde[$Lis_thn->periode] . " - " . $Lis_thn->thn;
                        }
                        $thn_a      = $thn;
        ...
        if ($this->input->is_ajax_request()) {
            try {
                ...
                if ($pro == '' && $rpjmn=='') {
                    ...
                    $max_pe_p    = "Pertumbuhan ekonomi " . $xname . " pada " . $categories[5] . " " . $nu_p . " dibandingkan dengan tahun " . $categories[4] . ". Pada " . $categories[5] . " pertumbuhan ekonomi " . $xname . " adalah " . $nilai_pro[5] . ", sedangkan pada " . $categories[4] . " pertumbuhannya tercatat sebesar " . $nilai_pro[4] . "%. </Br></Br> Pertumbuhan ekonomi " . $xname . " pada " . $categories[5] . " berada " . $ba_p . " nasional. Pertumbuhan ekonomi nasional pada " . $categories[5] . " adalah " . $nilai[5] . "%. ";
                    ...
                    $perbandingan_2th = "Perbandingan pertumbuhan ekonomi " . $categories_pro[4] . " dengan " . $categories_pro[5] . " menunjukkan bahwa 
                        provinsi yang mengalami penurunan pertumbuhan ekonomi terbesar adalah " . $label_tk_per_p_max . " (" . number_format($nilai_tk_per_p_min, 0, ",", ".") . " %). 
                        Dari sisi perubahan pertumbuhan ekonomi tahun " . $categories_pro[4] . " hingga " . $categories_pro[5] . ", " . $xname . " berada pada urutan ke-" . $rengkingtk_p . " , dengan pertumbuhan ekonomi " . $nu_p . " sebesar " . number_format($nper_tk, 0, ",", ".") . " %";
                    ...
                    */
                    $sql = "SELECT REF.id_periode,REF.periode,IFNULL(IND.nilai,0) nilai,IND.target,IND.t_m_rpjmn,IND.t_rkpd,IND.t_k_rkp, IFNULL(IND.sumber,'') sumber_k,REF.tahun 
                                FROM(
                                    ...
                                    WHERE id_indikator='1' AND rpjmn='".$dari."'  
                                    ) REF
                                LEFT JOIN(
                    ...
                    foreach ($list_data->result() as $row) {
                        ...
                        if($row->nilai==0 ){
                         ...
                         $nilai[]=$nilai1;
                        $periode                = $row->periode;
                        //$categories1[]          = $bulan[$row->periode]." ".$row->tahun; 
                        //$periode                = $row->periode;
                        if ($periode == '00') {
                            $thn = $row->tahun; 
                        } else {
                                ...
                                LEFT JOIN(
                                            ...
                                            WHERE (id_indikator='1' AND wilayah='".$query."')
                                                AND (id_periode, versi) IN (
                                                                SELECT id_periode, MAX(versi) AS versi 
                                                                FROM nilai_indikator 
                                                                WHERE id_indikator='1' AND wilayah='".$query."' GROUP BY id_periode
                                                ) 
                                            GROUP BY id_periode 
                                            ORDER BY id_periode DESC LIMIT 12
                                ) IND	ON REF.id_periode=IND.id_periode";
                ...
                $json_data = array(
                    ...
                    "text_kab"       => "Perbandingan Pertumbuhan Ekonomi Tahun " . $tahun_p_k . " Antar Kab/Kota " . $xname . "(%)",
