...
{
    var $view_dir   = "peppd1/upload_data/";
    var $js_init    = "main";
    var $js_path    = "assets/js/admin/upload_data/upload_data.js";
    ...
    public function index()
    {
        if ($this->input->is_ajax_request()) {
            try {
                ...
                //common properties
                $this->js_init    = "main";
                $this->js_path    = "assets/js/peppd1/upload_data/upload_data_" . $this->session->userdata(SESSION_LOGIN)->groupid . ".js?v=" . now("Asia/Jakarta");
    ...
    function add_act()
    {
        if ($this->input->is_ajax_request()) {
            try {
                ...
                $sql_n = 'SELECT MAX(id) AS idnomor FROM `nilai_indikator`';
                ...
                while (($data1 = fgetcsv($handle)) !== FALSE) {
                    if ($skip >= 2) {
                        ...
                        if (strlen($data1[2]) !== 2) {
                            throw new Exception("Penulisan Periode Salah Pada Baris ke " . $skip . ", Contoh penulisan periode: 03");
                        }
                        if (strlen($data1[3]) !== 6) {
                            throw new Exception("Penulisan Id Periode Salah Pada Baris ke " . $skip . "");
                        }

                        //                       if(strlen($data1[11]) !== 10){
                        //                           throw new Exception("Format Tanggal Salah Pada Baris ke ".$skip."");
                        //                       }
                        if (strlen($data1[7]) == 10) {
                            $pecah = explode("-", $data1[7]);
                            if (strlen($pecah[0]) != 4 || strlen($pecah[1]) != 2 || strlen($pecah[2]) != 2) {
                                throw new Exception("Format Tanggal Versi Salah Pada Baris ke " . $skip . "");
                            }
                        }
                        if (strlen($data1[10]) == '') {
                            throw new Exception("Id IndikatorKosong Pada Baris ke " . $skip . "");
                        }
                    }
                    $skip++;
                }
                while (($data = fgetcsv($handle1, 10000, ",")) !== FALSE) {
                    if ($skip1 >= 2) {
                        $nm = $urut + $noo++;
                        //$string = "SELECT * FROM `nilai_indikator` WHERE `wilayah`='".$data[0]."' AND id_periode ='".$data[3]."' AND versi='".$data[7]."' AND id_indikator='".$data[10]."'";
                        $string = "SELECT * FROM `nilai_indikator` "
                            . "WHERE `wilayah`  ='" . $data[0] . "' "
                            . "AND periode   ='" . $data[2] . "' "
                            . "AND id_periode   ='" . $data[3] . "' "
                            . "AND tahun   ='" . $data[1] . "' "
                            . "AND versi        ='" . $data[10] . "' "
                            . "AND id_indikator ='" . $data[13] . "' ";

                        $q = $this->db->query($string);
                        if ($q->num_rows() == 0) {
                            $this->m_ref->setTableName("nilai_indikator");
                            $data_baru = array(
                                "id"            => $nm,
                                "wilayah"       => $data[0],
                                "tahun"         => $data[1],
                                ...
                                "nilai"         => $data[4],
                                "nasional"      => $data[5],
                                "target"        => (empty($data[6]) ? NULL : $data[6]),
                                "t_m_rpjmn"     => (empty($data[7]) ? NULL : $data[7]),
                                "t_rkpd"        => (empty($data[8]) ? NULL : $data[8]),
                                "t_k_rkp"       => (empty($data[9]) ? NULL : $data[9]),
                                "versi"         => $data[10],
                                "satuan"        => $data[11],
                                "sumber"        => $data[12],
                                "id_indikator"  => $data[13],
                                "nama_indikator" => $data[14],
                                "keterangan"    => '',
                            );
                            ...
                            $string_d = "DELETE FROM `nilai_indikator` "
                                . "WHERE `wilayah`='" . $data[0] . "' "
                                . "AND periode   ='" . $data[2] . "' "
                                . "AND id_periode   ='" . $data[3] . "' "
                                . "AND tahun   ='" . $data[1] . "' "
                            ...
                            //                                "id_periode"    => $data[3],
                            //                                "versi"         => $data[7],
                            //                                "id_indikator"  => $data[10],
                            //                            );
                            //                            $status_save = $this->m_ref->update($cond,$data_baru);
                            //                            if(!$status_save){
                            //                                log_message("error", $this->db->error()["message"]);
                            //                                throw new Exception($this->db->error()["code"].":Failed update data1",0);
                            //                            }
