...
class Laporan_3 extends CI_Controller {
    var $view_dir   = "admin/laporan_3/";
    var $js_init    = "main";
    var $js_path    = "assets/js/admin/laporan_3/laporan_3.js";
...
//        require_once (APPPATH.'/third_party/jpgraph-4.3.1/src/jpgraph_iconplot.php');
        //require_once (APPPATH.'/third_party/PHPWord-develop/bootstrap.php');
    ...
    public function index(){
        if($this->input->is_ajax_request()){
            ...
            {                
                ...
                //common properties
                $this->js_init    = "main";
                $this->js_path    = "assets/js/admin/laporan_3/laporan_3.js";
                $now=date('Y');
                $data_page = array(
                    "now" => $now
                );
...
//$gbplot_kab_per->value->SetFormat('%01.2f');  
               ...
               $select_pdrb="SELECT R.id,R.nama_indikator,IFNULL(IND.nilai,0) nilai_struktur,IFNULL(IND.target,0) nilai_pertumbuhan  
                ...
                foreach ($list_s_p->result() as $row_s_p) {
                     ...
                     $nilai_s[]     = $row_s_p->nilai_struktur;
                ...
                $pdrb_d="Tiga sektor yang paling dominan dalam struktur perekonomian ". $xname ." adalah sektor "
                        . "Pertanian, Kehutanan, dan Perikanan dengan kontribusi sebesar ".$nilai_max_s." persen, diikuti oleh sektor Administrasi Pemerintahan, Pertahanan dan Jaminan Sosial Wajib dengan kontribusi sebesar 22,4 persen, kemudian sektor Perdagangan Besar dan Eceran; Reparasi Mobil dan Sepeda Motor dengan kontribusi sebesar 13,78 persen. "
                        . "Sedangkan tiga sektor yang memiliki pertumbuhan paling tinggi adalah sektor Pengadaan Listrik dan Gas dengan pertumbuhan sebesar ".$nilai_max_p." persen, "
                        . "diikuti oleh sektor Jasa Kesehatan dan Kegiatan Sosial dengan pertumbuhan sebesar 6,92 persen, "
                        . "serta sektor Jasa Pendidikan dengan pertumbuhan sebesar 5,42 persen. Di sisi lain tiga sektor yang mengalami pertumbuhan terendah adalah sektor Pertambangan dan Penggalian dengan pertumbuhan sebesar -2,06 persen, diikuti oleh sektor Real Estat dengan pertumbuhan sebesar 0,51 persen, serta sektor Informasi dan Komunikasi dengan pertumbuhan sebesar 0,53 persen.					";
                //print_r($select_pdrb);exit();
...
//                $this->m_pdf->pdf->setFooter('{PAGENO}','','{DATE j-m-Y}');
    ...
    function download_act() {
        ...
        //$section->addSection(array('pageNumberingStart' => 1));

        // Save File / Download (Download dialog, prompt user to save or simply open it)
        $section->addText('Evaluasi Kinerja Pencapaian MakroPembangunan ', 'rStyle', 'pStyle');
        $section->addText("$xname", 'rStyle', 'pStyle');
        $section->addTextBreak(30);
                
        $section->addText('Pertumbuhan Ekonomi', 'rStyle');
        $section->addText("$peek_d", array('name'=>'calibri'));
        $section->addText("$max_pe", array('name'=>'calibri'));
        $section->addText("$max_pe_p", array('name'=>'calibri'));
        $section->addText("$pe_rkpd_rkp", array('name'=>'calibri'));
     
      



        $filename=$provinsi.'.docx'; //s
        //ave our document as this file name
...
$section1->addTitle('2D charts', 1);
$section1 = $phpWord->addSection(array('colsNum' => 2, 'breakType' => 'continuous'));
