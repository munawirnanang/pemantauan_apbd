...
class Tingkat_pengangguran_terbuka extends CI_Controller {
    var $view_dir   = "peppd1/evaluasi/tingkat_pengangguran_terbuka/";
    var $js_init    = "main";
    var $js_path    = "assets/js/admin/tingkat_pengangguran_terbuka/tingkat_pengangguran_terbuka.js";
    ...
    public function index(){
        if($this->input->is_ajax_request()){
            ...
            {                
                ...
                //common properties
                $this->js_init    = "main";
                $this->js_path    = "assets/js/peppd1/evaluasi/tingkat_pengangguran_terbuka/tingkat_pengangguran_terbuka_".$this->session->userdata(SESSION_LOGIN)->groupid.".js?v=".now("Asia/Jakarta");
    ...
    function peta(){
        if($this->input->is_ajax_request()){
            try {
                ...
                $posisi  = array('1100' => [96.699389, 4.713777], '1200' => [99.505662, 2.214178],'1300' => [101.030895, -0.709370],'1400' => [101.694295, 0.337480],'1500' => [102.447403, -1.444049],'1600' => [104.090464, -3.196092],'1700' => [102.234200, -3.574566],'1800' => [105.149064, -4.874095],'1900' => [106.841340, -2.303002],'2100' => [104.629934, 0.883574],'3100' => [106.850787, -6.206519],'3200' => [107.768762, -6.942884],'3300' => [109.998987, -7.313526],'3400' => [110.462457, -7.850908],'3500' => [112.644437, -7.585143],'3600' => [106.120118, -6.373885],'5100' => [115.183905, -8.408885],'5200' => [117.364766, -8.653642],'5300' => [122.367551, -9.007799],'6100' => [111.154836, -0.081797],'6200' => [113.538086, -1.591910],'6300' => [115.511801, -2.994677],'6400' => [116.491959, 0.592887],'6500' => [116.252830, 3.157217],'7100' => [124.464421, 0.943605],'7200' => [120.985712, -0.824342],'7300' => [119.922324, -3.585060],'7400' => [122.263216, -4.009943],'7500' => [122.509390, 0.701081],'7600' => [119.285441, -2.450009],'8100' => [130.535005, -3.598017],'8200' => [127.826539, 0.742708],'9100' => [132.783737, -1.926649],'9400' => [138.775450, -4.486719]);
                                
                $xname      ="";
                $query      ="";
                //$title_y    ="Pertumbuhan Ekonomi(%)";
                $title      ="Perkembangan Tingkat Pengangguran Terbuka tahun";
                $thn_a      ='';
                $max_pe_p   ='';
                $pe_rkpd_rkp='';
                $kord       ='';
                ...
                if($pro == '' ){
                    ...
                    $nsl = "select id_periode,nilai, max(versi) as versi from nilai_indikator WHERE id_indikator='6' AND wilayah='1000' ";
                       ...
                       $type1[] = ["type"        => "MultiPolygon",
                                    "type" => "Feature",
                                        ...
                                        "properties"  =>array("ID"   =>'Indonesia',
                                                         ...
                                                         "description"=>
                                                                '<strong>'.$row_ppe_per->nama_provinsi.'</strong><p> Tingkat Pengangguran Terbuka : '.$row_ppe_per->nilai.' %</p>',
                                         ),
                                        ];
                       $nilai_peta = $type1;
                    }
                    $thnmax ="SELECT MAX(tahun) AS thn,sumber,periode FROM nilai_indikator WHERE `wilayah`='1000' AND id_indikator = '6' "; 
                    $list_thn = $this->db->query($thnmax);
                    foreach ($list_thn->result() as $Lis_thn){
                    $periode                = $Lis_thn->periode;
                    if($periode == '00'){ $thn=$Lis_thn->thn;}
                    else{ $thn=  $bulan[$Lis_thn->periode]." - ".$Lis_thn->thn; }
                    $thn_a=$thn;
...
//                    array_push($catdata, $nilaiData);
                    ...
                    $perbandingan_pe ="select p.label as label,p.nama_provinsi, e.* 
                                    from provinsi p 
                                    join nilai_indikator e on p.id = e.wilayah 
                                    where (e.id_indikator='6' AND e.id_periode='$periode_pe_max') 
                                    AND (wilayah, versi) in (select x.wilayah, max(x.versi) as versi 
							from nilai_indikator x  
							where id_indikator='6' AND id_periode='$periode_pe_max' 
                            group by wilayah) group by wilayah order by wilayah asc                    ";
                    ...
                    //perbandingan kab
                    $catdata_kab = array();
                    $th_p_kab ="select max(e.id_periode) AS perio from kabupaten p join nilai_indikator e on p.id = e.wilayah where p.prov_id='".$query."' AND e.id_indikator='6' ";
                    ...
                    $ppe_kab="select p.nama_kabupaten as label,p.prov_id, e.* 
                            from kabupaten p
                            join nilai_indikator e on p.id = e.wilayah 
                            where p.prov_id='".$query."' and (e.id_indikator='6' AND e.id_periode='$perio') AND (wilayah, versi) in (
                    ...
                    foreach ($list_kab_ppe_per->result() as $row_ppe_kab_per) {
                        ...
                        $type[] = ["type"        => "MultiPolygon",
                                        ...
                                        "properties"  =>array("ID"   =>'Indonesia',
                                                         ...
                                                         "description"=>
                                                                '<strong>'.$label_ppe11.'</strong><p></Br> Periode : '.$periode_kab.' </Br> Tingkat Pengangguran Terbuka : '.number_format($row_ppe_kab_per->nilai,2,",",".").' %</p>',
                                         ),
                                        ];
                       $nilai_peta = $type;
                    }
    ...
    function tingkat_pengangguran_terbuka(){
        if($this->input->is_ajax_request()){
            try {
                $requestData= $_REQUEST;
                $this->form_validation->set_rules('provinsi','Provinsi','required');
                ...
                if($pro == '' && $rpjmn=='' ){
                    ...
                    $max_pe_p    ="Tingkat pengangguran terbuka ". $xname ." pada ".$categories[5]." ".$nu_p." dibandingkan ".$categories[3].". Pada ".$categories[5]." tingkat pengangguran terbuka ". $xname ." adalah sebesar ". number_format(end($nilai_pro),2,",",".") ."%, sedangkan pada ".$categories[3]." tingkat pengangguran terbuka tercatat sebesar ".number_format($nilai_pro[3],2,",",".")."%.";
                    //$pe_rkpd_rkp ="Tingkat pengangguran terbuka ". $xname ." pada ".$categories[5]." capaian nasional. Tingkat pengangguran terbuka nasional pada ".$categories[5]." adalah sebesar target RKPD ". $xname ." (".$nilai_rkpd1[5]."%) dan ".$rkppe." target kewilayahan RKP (".$nilai_rkp1[5]."%).";
           ...
           IFNULL(IND.sumber,'') sumber_k,IND.tahun 
                    ...
                    $pe_perbandingan_pro = "Perbandingan tingkat pengangguran terbuka antar 34 provinsi menunjukkan bahwa tingkat pengangguran terbuka ". $xname ." 
                        pada ".$tahun_tk_p_max." berada pada urutan ke-".$rengking_pro.".
                           Provinsi dengan tingkat pengangguran terbuka adalah ".array_shift($nama_pe2)." (".$nilai_ppe_p_max."%),
                            sedangkan provinsi dengan tingkat pengangguran terbuka terendah adalah ".end($nama_pe2)." (".$nilai_ppe_p_min."%). ";
           ...
           IFNULL(IND.sumber,'') sumber_k,IND.tahun 
					...
					FROM provinsi p 
					JOIN nilai_indikator e ON p.id = e.wilayah 
					WHERE (e.id_indikator='6' AND e.id_periode='$periode_pe[3]') 
						AND (wilayah, versi) IN (
							SELECT x.wilayah, MAX(x.versi) AS versi 
							FROM nilai_indikator X 
							WHERE id_indikator='6' AND id_periode='$periode_pe[3]' 
							GROUP BY wilayah ) 
					GROUP BY wilayah
                                ) IND	ON REF.id=IND.wilayah ORDER BY id  ASC";
                    $list_radar = $this->db->query($perbandingan_tk2);
                    foreach ($list_radar->result() as $row_radar) {
                        $label_tk2[]   = $row_radar->label;
...
//                    $label_ppe1          = $label_ppe;
                    ...
                    $nilaiData_r2['name'] = $tahun_tk_p_2;
                    $nilaiData_r2['data'] = $nilai_ppe_per2;
                    array_push($catdata_radar, $nilaiData_r2);
...
//                
                ...
                $pe_perbandingan_radar = "Perbandingan tingkat pengangguran terbuka ".$categories[3]." dengan ".$categories[5]." menunjukkan bahwa 
                        provinsi yang mengalami penurunan tingkat pengangguran terbuka terbesar adalah ".$label_tk_per_p_max." (".number_format($nilai_tk_per_p_min,0,",",".")." orang). 
                        Dari sisi perubahan tingkat pengangguran terbuka periode ".$categories[3]." hingga ".$categories[5].", ". $xname ." berada pada urutan ke-".$rengkingtk_p.", dengan tingkat pengangguran terbuka ".$nu_p." sebesar ".number_format($nper_tk,0,",",".")."%";
                   ...
                   $pe_perbandingan_kabb="Perbandingan tingkat pengangguran terbuka antar kabupaten/kota di ". $xname ." memperlihatkan 
                                            bahwa pada periode ".$tahun_p_k." daerah dengan tingkat tingkat pengangguran terbuka tertinggi adalah ".array_shift($nama_k2)." (".number_format($nilai_ppe_per_kab_max,2,",",".")."%), sedangkan daerah dengan tingkat pengangguran terbuka terendah adalah ".end($nama_k2)." (".number_format($nilai_ppe_per_kab_min,2,",",".")."%).
                                            Selisih tingkat pengangguran terbuka tertinggi dan terendah di ". $xname ." pada periode ".$tahun_p_k." adalah sebesar ".$selisih_pe."%.";
                    ...
                    $sampai = $rpjmn + 4;
                    $catdata = array();
                    $nsl = "select  max(id_periode) as periode_id from nilai_indikator WHERE id_indikator='6' AND wilayah='1000' AND tahun BETWEEN '".$dari."' AND '".$sampai."' ";
                    $list_nsl  = $this->db->query($nsl);
                    foreach ($list_nsl->result() as $row_n) {
...
//                                    from nilai_indikator 
//                                    WHERE id_indikator='36' AND wilayah='1000' 
//                                    AND tahun BETWEEN '".$dari."' AND '".$sampai."'
//                                    group by id_periode) 
//                                group by id_periode 
//                                order by id_periode desc limit 10) y 
//                            order by id_periode ASC";
                    $sql = "SELECT REF.id_periode,REF.periode,IFNULL(IND.nilai,0) nilai,IND.target,IND.t_m_rpjmn,IND.t_rkpd,IND.t_k_rkp, IFNULL(IND.sumber,'') sumber_k,REF.tahun 
                                FROM(
                                    ...
                                    WHERE id_indikator='6' AND rpjmn='".$dari."'  
                                    ) REF
                                LEFT JOIN(
                    ...
                    */
                        $sql_dpro = "SELECT REF.id_periode,REF.periode,IFNULL(IND.nilai,0) nilai,IND.target,IND.t_m_rpjmn,IND.t_rkpd,IND.t_k_rkp, IFNULL(IND.sumber,'') sumber_k,REF.tahun 
                                ...
                                LEFT JOIN(
                                            SELECT id_periode,periode,nilai,target,t_m_rpjmn,t_rkpd,t_k_rkp,sumber,tahun 
                                            FROM nilai_indikator 
                                            WHERE (id_indikator='6' AND wilayah='".$query."')
                                                AND (id_periode, versi) IN (
                                                                SELECT id_periode, MAX(versi) AS versi 
                                                                FROM nilai_indikator 
                                                                WHERE id_indikator='6' AND wilayah='".$query."' GROUP BY id_periode
                                                ) 
                                            GROUP BY id_periode 
                                            ORDER BY id_periode DESC LIMIT 12
                                ) IND	ON REF.id_periode=IND.id_periode";
...
//                    $pe_rkpd_rkp ="Tingkat pengangguran terbuka ". $xname ." pada ".$categories[5]." capaian nasional. tingkat pengangguran terbuka nasional pada ".$categories[5]." adalah sebesar target RKPD ". $xname ." (".$nilai_rkpd1[5]."%) dan ".$rkppe." target kewilayahan RKP (".$nilai_rkp1[5]."%).";
                ...
                $json_data = array(
                    ...
                    "text_pro"       => "Perbandingan Tingkat Pengangguran Terbuka ".$tahun_tk_p_max." Antar Provinsi (%)",
                    "categories_pro"=> $label_ppe1,
                    "series_pro"     => $catdata_pro,
                    "pe_perbandingan_pro" =>$pe_perbandingan_pro,
//                    
                    "text_radar"       => "Perbandingan Tingkat Pengangguran Terbuka ".$tahun_tk_p_max." Dengan ".$tahun_tk_p_2." (%)",
                    "tahun_1" => $tahun_tk_p_max,
                    "catdata_radar" => $catdata_radar,
                    "perbandingan_radar" =>$pe_perbandingan_radar,
//                    
                    "text_kab"       => "Perbandingan Tingkat Pengangguran Terbuka ".$tahun_pe_kab." (%)",
