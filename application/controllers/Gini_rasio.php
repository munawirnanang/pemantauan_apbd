...
class Gini_rasio extends CI_Controller {
    var $view_dir   = "PEPPD1/evaluasi/gini_rasio/";
    var $js_init    = "main";
    var $js_path    = "assets/js/admin/gini_rasio/gini_rasio.js";
        ...
        if($this->input->is_ajax_request()){
            ...
            {                
               ...
               // print_r($current_date_time);exit();
                //common properties
                $this->js_init    = "main";
                $this->js_path    = "assets/js/peppd1/evaluasi/gini_rasio/gini_rasio_".$this->session->userdata(SESSION_LOGIN)->groupid.".js?v=".now("Asia/Jakarta");
...
//                $string_provinsi = " P.`nama_provinsi` IN(";
                ...
                $query      ="";
                $tahun_tk_p_max="";
                $title_y    = "Perkembangan Gini Rasio";
                $title="Perkembangan Gini Rasio";
                $select_title="SELECT * FROM indikator where id='7'";
                $list_title = $this->db->query($select_title);
                foreach($list_title->result() as $lst){
                    //$title= $lst->nama_indikator;
                    
                }
                $d_peek="SELECT I.`deskripsi` FROM indikator I where id='7'";
                $list_peek = $this->db->query($d_peek);
                foreach ($list_peek->result() as $peek){$peek_d   = $peek->deskripsi;}
                $max_pe_p='';
                $gr_n='';
                $pe_rkpd_rkp='';
                $pe_perbandingan_pro='';
                $gr_perbandingan_kabb='';
                if($pro == '' ){
                    $judul="Perkembangan Gini Rasio";
                    ...
                    foreach ($list_data->result() as $Lis_pro){
                        $xname = $Lis_pro->nama_provinsi;
                        $query = $Lis_pro->id;
                        $judul = "Perkembangan Gini Rasio ".$Lis_pro->nama_provinsi;
                        $label_pe = $Lis_pro->label;
                    }
                    $sql = "SELECT * FROM (select * from nilai_indikator where (id_indikator='7' AND wilayah='1000') AND (id_periode, versi) in (select id_periode, max(versi) as versi from nilai_indikator WHERE id_indikator='7' AND wilayah='1000' group by id_periode) group by id_periode order by id_periode desc limit 6) y order by id_periode ASC"; 
                    $list_data  = $this->db->query($sql);
...
//                    }
                    if($nilai_rkpd1[5]=='-'){
                        ...
                        if($nilai_pro[5] > $nilai_rkpd1[5]){ $rkpdgr="diatas"; }
                        else{ $rkpdgr="dibawah"; }
                        $gr_rkpd_rkp1 = "Gini rasio ". $xname ." pada ".$categories[5]." berada ".$rkpdgr." target RKPD ". $xname ."(".$nilai_rkpd1[5].").";
                    }
                    
                    if($nilai_pro[5] > $nilai_rkp1[5]){ $rkppe='di atas'; }
                    else{ $rkppe='di bawah'; }
                    $max_pe_p    ="Gini rasio ". $xname ." pada ".$categories[5]." ".$nu_p." dibandingkan dengan ".$categories[3].". Pada ".$categories[5]." gini rasio ". $xname ." adalah sebesar ". number_format(end($nilai_pro),3,",",".") .", sedangkan pada ".$categories[3]." gini rasio tercatat sebesar ".number_format($nilai_pro[3],3,",",".").".";
                    $gr_n ="Gini rasio ". $xname ." pada ".$categories[5]." berada ... capaian nasional. Gini rasio nasional pada ".$categories[5]." adalah sebesar ...  ";
                    $pe_rkpd_rkp =$gr_rkpd_rkp1;
                    $periode_pe_max=max($periode_pe);
                    $catdata_pro = array();
                    $perbandingan_pe ="select p.label as label,p.nama_provinsi, e.* 
                                    from provinsi p 
                                    join nilai_indikator e on p.id = e.wilayah 
                                    where (e.id_indikator='7' AND e.id_periode='$periode_pe_max') 
                                    AND (wilayah, versi) in (select x.wilayah, max(x.versi) as versi 
							from nilai_indikator x  
							where id_indikator='7' AND id_periode='$periode_pe_max' 
                            group by wilayah) group by wilayah order by wilayah asc                    ";
                    $list_ppe_per = $this->db->query($perbandingan_pe);
                    foreach ($list_ppe_per->result() as $row_ppe_per) {
                        $label_ppe[]                               = $row_ppe_per->label;
                    ...
                    arsort($nama_pe1);
                    $nama_pe2=array_keys($nama_pe1);
                    $pe_perbandingan_pro = "Perbandingan gini rasio antar 34 provinsi menunjukkan bahwa gini rasio ". $xname ." 
                        pada ".$tahun_tk_p_max." berada pada urutan ke-".$rengking_pro.".
                           Provinsi dengan gini rasio tertinggi adalah ".array_shift($nama_pe2)." (".$nilai_ppe_p_max."),
                            sedangkan yang terendah adalah ".end($nama_pe2)." (".$nilai_ppe_p_min."). ";
                    
                    //perbandingan kab
                    $catdata_kab = array();
                    $th_p_kab ="select max(e.id_periode) AS perio from kabupaten p join nilai_indikator e on p.id = e.wilayah where p.prov_id='".$query."' AND e.id_indikator='7' ";
                    $t_list_kab_pe = $this->db->query($th_p_kab);
                    foreach ($t_list_kab_pe->result() as $row_t_pe_kab) {
                        $perio = $row_t_pe_kab->perio;
                    }
                    $ppe_kab="select p.nama_kabupaten as label, e.* 
                            from kabupaten p
                            join nilai_indikator e on p.id = e.wilayah 
                            where p.prov_id='".$query."' and (e.id_indikator='7' AND e.id_periode='$perio') AND (wilayah, versi) in (
                               select x.wilayah, max(x.versi) as versi from nilai_indikator x  where id_indikator='7' AND id_periode='$perio' group by wilayah ) 
                           group by wilayah order by wilayah asc";
                    //print_r($ppe_kab);exit();
                    $list_kab_ppe_per = $this->db->query($ppe_kab);
                    foreach ($list_kab_ppe_per->result() as $row_ppe_kab_per) {
                   ...
                   $tahun_pe_kab=$tahun_p_k." Antar Kabupaten/kota di ".$judul ;
                   $gr_perbandingan_kabb="Perbandingan gini rasio antar kabupaten/kota di ". $xname ." memperlihatkan 
                                            bahwa pada tahun ".$tahun_p_k." daerah dengan gini rasio tertinggi adalah ".array_shift($nama_k2)." (".$nilai_ppe_per_kab_max." ), "
                           . "sedangkan daerah dengan gini rasio terendah adalah ".end($nama_k2)." (".$nilai_ppe_per_kab_min.").
                                            Selisih tingkat gini rasio tertinggi dan terendah di ". $xname ." Pada periode ".$tahun_p_k." adalah sebesar ".$selisih_pe."%.";
                ...
                $json_data = array(
                    ...
                    "text_pro"       => "Perbandingan Gini Rasio ".$tahun_tk_p_max." Antar Provinsi (%)",
                    "categories_pro"=> $label_ppe1,
                    "series_pro"     => $catdata_pro,
                    "pe_perbandingan_pro" =>$pe_perbandingan_pro,
                    
                    "text_kab"       => "Perbandingan Gini Rasio Antar Kab (%)",
                    "categories_kab" => $label_pe_k1,
                    "series_kab"     => $catdata_kab,
                    "tahun_pe_kab"   => $tahun_pe_kab,
                    "gr_perbandingan_kab" =>$gr_perbandingan_kabb,
                );
                exit(json_encode($json_data));
            } catch (Exception $exc) {
                $json_data = array(
