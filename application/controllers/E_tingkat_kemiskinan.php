...
class E_tingkat_kemiskinan extends CI_Controller {
    var $view_dir   = "peppd1/evaluasi/tingkat_kemiskinan/";
    var $js_init    = "main";
    ...
    /*
    * Evaluasi Kinerja Indikator Tingkat Kemiskinan
    * author : Fsm
    * date  : 
    */
    public function index(){
        if($this->input->is_ajax_request()){
            ...
            {                
                ...
                //common properties
                $this->js_init    = "main";
                $this->js_path    = "assets/js/peppd1/evaluasi/tingkat_kemiskinan/tingkat_kemiskinan_".$this->session->userdata(SESSION_LOGIN)->groupid.".js?v=".now("Asia/Jakarta");
    ...
    function pro_datatable(){
        if($this->input->is_ajax_request()){
            try {
                ...
                $sql = "SELECT A.id,A.`nama_provinsi`,A.`label`,A.`ppd` FROM provinsi A WHERE 1=1 ";
    ...
    function tinkat_kemiskinan(){
        if($this->input->is_ajax_request()){
            try {
                ...
                $title              ="Perkembangan Tingkat Kemiskinan";
                ...
                if($pro == '' && $rpjmn=='' ){
                    ...
                    * =========================================================================
                    */
                    $perbandingan_pe ="SELECT REF.id,REF.label AS label,REF.nama_provinsi, IND.id_periode,IND.periode,IFNULL(IND.nilai,0) nilai,IND.target,IND.t_m_rpjmn,IND.t_rkpd,IND.t_k_rkp, IFNULL(IND.sumber,'') sumber_k,IND.tahun 
                                        FROM( SELECT `id`,`label`,`nama_provinsi` FROM `provinsi`  WHERE 1=1 ORDER BY id  ASC ) REF
                                        LEFT JOIN(
                                        SELECT p.label AS label,p.nama_provinsi, e.* 
                                        FROM provinsi p 
                                        JOIN nilai_indikator e ON p.id = e.wilayah 
                                        WHERE (e.id_indikator='36' AND e.id_periode='$periode_pe_max')  AND (wilayah, versi) IN ( SELECT wilayah, MAX(versi) AS versi  FROM nilai_indikator WHERE id_indikator='36' AND id_periode='$periode_pe_max' 
                                        GROUP BY wilayah ) GROUP BY e.wilayah ) IND	ON REF.id=IND.wilayah ORDER BY id  ASC";
                ...
                $perbandingan_2th = "Perbandingan tingkat kemiskinan ".$tahun_tk_p_2." dengan ".$tahun_tk_p_max." menunjukkan bahwa 
                        ...
                        Dari sisi perubahan tingkat kemiskinan ".$tahun_tk_p_2." hingga ".$tahun_tk_p_max.", ". $xname ." berada pada urutan ke-".$rengkingtk_p." , dengan tingkat kemiskinan ".$nu_p." sebesar ".number_format($nper_tk,2,",",".")." %";
                    ...
                    */
                    ...
                    foreach ($list_kab_ppe_per->result() as $row_ppe_kab_per) {
                        ...
                        if ($posisi_ppe !== FALSE){ $label_ppe11 = substr( $row_ppe_kab_per->label,0,3).". ".substr( $row_ppe_kab_per->label,10); }
                        else{ $label_ppe11 = $row_ppe_kab_per->label; }
                    ...
                    foreach($nilai_ppe_per_kab as $xk=>$xk_value){
                        if($xk==$label_pe){ $rengking_pro_k=$nr++; }
                        $urutan_pro_k=$xk . $xk_value . $xk_value .$nrk++;
                    }
                   $tahun_pe_kab=$tahun_p_k." Antar Kabupaten/kota di ".$xname ;
                   $pe_perbandingan_kabb ="Perbandingan tingkat kemiskinan antar kabupaten/kota di ". $xname ." memperlihatkan bahwa pada tahun ".$tahun_p_k." daerah dengan tingkat tingkat kemiskinan tertinggi adalah ".array_shift($nama_k2)." (".$nilai_ppe_per_kab_max." %), sedangkan daerah dengan tingkat kemiskinanmi terendah adalah ".end($nama_k2)." (".$nilai_ppe_per_kab_min."%). Selisih tingkat kemiskinan tertinggi dan terendah di ". $xname ." pada tahun ".$tahun_p_k." adalah sebesar ".$selisih_pe."%.";
                    ...
                    $sampai = $rpjmn + 4;
                    $catdata = array();
                    $nsl = "select  max(id_periode) as periode_id from nilai_indikator WHERE id_indikator='36' AND wilayah='1000' AND tahun BETWEEN '".$dari."' AND '".$sampai."' ";
                    ...
                    */
                    $sql = "SELECT REF.id_periode,REF.periode,IFNULL(IND.nilai,0) nilai,IND.target,IND.t_m_rpjmn,IND.t_rkpd,IND.t_k_rkp, IFNULL(IND.sumber,'') sumber_k,REF.tahun 
                                FROM(
                                    ...
                                    WHERE id_indikator='36' AND rpjmn='".$dari."'  
                                    ) REF
                                LEFT JOIN(
                    ...
                    */
                        ...
                        $sql_dpro = "SELECT REF.id_periode,REF.periode,IFNULL(IND.nilai,0) nilai,IND.target,IND.t_m_rpjmn,IND.t_rkpd,IND.t_k_rkp, IFNULL(IND.sumber,'') sumber_k,REF.tahun 
                                ...
                                LEFT JOIN(
                                            SELECT id_periode,periode,nilai,target,t_m_rpjmn,t_rkpd,t_k_rkp,sumber,tahun 
                                            FROM nilai_indikator 
                                            WHERE (id_indikator='36' AND wilayah='".$query."')
                                                AND (id_periode, versi) IN (
                                                                SELECT id_periode, MAX(versi) AS versi 
                                                                FROM nilai_indikator 
                                                                WHERE id_indikator='36' AND wilayah='".$query."' GROUP BY id_periode
                                                ) 
                                            GROUP BY id_periode 
                                            ORDER BY id_periode DESC LIMIT 12
                                ) IND	ON REF.id_periode=IND.id_periode";
                ...
                $json_data = array(
                    ...
                    "text_radar"            => "Perbandingan Tingkat Kemiskinan ".$tahun_tk_p_2." Dengan Periode ".$tahun_tk_p_max." (%)",
                    "tahun_1"               => $tahun_tk_p_max,
                    "catdata_radar"         => $catdata_radar,
                    "perbandingan_2th"      => $perbandingan_2th,
                
                    "text_kab"              => "Perbandingan Tingkat Kemiskinan ".$tahun_pe_kab." (%)",
