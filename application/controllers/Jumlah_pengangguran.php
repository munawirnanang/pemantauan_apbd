...
class Jumlah_pengangguran extends CI_Controller {
            ...
            try {
                ...
                if($pro == ''){
                        ...
                        SELECT * 
                                                ...
                                                WHERE (id_indikator='4' AND wilayah='1000') 
                                                        AND (id_periode) IN (
                                                                ...
                                                                FROM nilai_indikator WHERE id_indikator='4' AND wilayah='1000' 
                                                                ) 
                                                GROUP BY id_periode 
                    ...
                    foreach ($list_ppe_per->result() as $row_ppe_per) {
                       ...
                       $type1[] = [
                           ...
                           "properties"  =>array(
                               ...
                               "description" => '<strong>'.$row_ppe_per->nama_provinsi.' - Periode : '.$thn_a.'</strong><p> Jumlah Pengangguran : '.number_format($row_ppe_per->nilai,0,",",".").' Orang</p>',
                               ),
                           ];
                       $nilai_peta = $type1;
                    }
                    ...
                    //nasional                        
                     $thnmax ="SELECT * FROM (
                                ...
                                WHERE (id_indikator='4' AND wilayah='".$query."') 
                                        AND (id_periode) IN (
                                                SELECT  MAX(id_periode) AS id_periode 
                                                FROM nilai_indikator WHERE id_indikator='4' AND wilayah='".$query."' 
                                                ) 
                                GROUP BY id_periode 
                        ORDER BY id_periode DESC LIMIT 6) Y 
                        ORDER BY id_periode ASC ";
                    ...
                    foreach ($list_thn->result() as $Lis_thn){
                        ...
                        $nilainsl = $xname." ".number_format($Lis_thn->nilai,0,",",".");
                        $thn_a           = $bln[$Lis_thn->periode]." ".$Lis_thn->tahun;
                        $datasumber ="Sumber : ".$Lis_thn->sumber;
                    }     
                    
                    $sql_dpro = "SELECT * FROM (select * from nilai_indikator where (id_indikator='4' AND wilayah='".$query."') 
                            AND (id_periode, versi) in (
                                select id_periode, max(versi) as versi 
                                from nilai_indikator 
                                WHERE id_indikator='4' AND wilayah='".$query."' group by id_periode) group by id_periode order by id_periode desc limit 6) y order by id_periode ASC"; 
                    ...
                    foreach ($list_kab_ppe_per->result() as $row_ppe_kab_per) {
                        ...
                        $type[] = ["type"        => "MultiPolygon",
                                        ...
                                        "properties"  =>array("ID"   =>$no++,
                                                              ...
                                                              "description"=>
                                                            '<strong>'.$label_ppe11.'</strong></Br> Periode : '.$periode_kab.' </Br><p> Jumlah Pengangguran : '.number_format($row_ppe_kab_per->nilai,0,",",".").' Orang</p>',
                                         ),
                                        ];
                       $nilai_peta = $type;
                    }
